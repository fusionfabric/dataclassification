...
from DataClassification.BusinessRulesEngine.DataContracts import RulesList, DefinitionList, Rule, Definition
...
class ClassificationEngine:
    ...
    def __validate_rules(self,root_folder):
        ...
        if len(incorrect_rules) > 0:
            logging.exception("The following names in the rules section don't have a definition: {}".format(incorrect_rules))
            raise TypeError("The following names in the rules section don't have a definition: {}".format(incorrect_rules))
    ...
    def __convert_technical_names_to_definintion_names(self, tech_names_list):
        ...
        for tn in tech_names_list:
            definition_names.append(self.dl.get_definition_name_by_technical_name(api_directory, tn))
        return definition_names

    def get_combination_tags(self, combination_list = []):
        tags = []
        combination_names_list = self.__convert_technical_names_to_definintion_names(combination_list)
        for rule in self.rl.rules_list:
            match_all_list = []
            match_any_list = []
            ...
            if len(match_all_list) == len(rule.all) and len(match_any_list) <= len(rule.any) and (len(match_all_list) > 0 or len(match_any_list) > 0) :
    ...
    def map_technical_names_to_definintion_names(self, api_directory, tech_names_list):
        ...
        for tn in tech_names_list:
            if ":" in tn:
                ...
                dn = self.dl.get_definition_name_by_technical_name(api_directory_new, tn)
                if dn != None:
                    definition_map[tn] = self.dl.get_definition_name_by_technical_name(api_directory_new, tn)
            else:
                dn = self.dl.get_definition_name_by_technical_name(api_directory, tn)
                if dn != None:
                    definition_map[tn] = self.dl.get_definition_name_by_technical_name(api_directory, tn)
        return definition_map

    def get_tags(self, api_directory, combination_list = []):
        mapped_tags = {}
        combination_names_list = self.map_technical_names_to_definintion_names(api_directory, combination_list)
        for rule in self.rl.rules_list:
            match_all_list = []
            match_any_list = []
            match_all_dict = {}
            ...
            if len(match_all_dict) == len(rule.all) and len(match_any_dict) <= len(rule.any) and (len(match_any_dict) > 0 or len(match_all_dict) > 0) :
    ...
    def get_unslassified_technical_names(self, combination_list = []):
        unclassified_list = []
        api_directory = []
        definition_names = self.map_technical_names_to_definintion_names(api_directory, combination_list)
