...
class ClassificationEngine:
    ...
    def __init__(self, root_folder, json_file_name):
        ...
        logging.basicConfig(filename=root_folder + '/data-classification.log',
    ...
    def __validate_rules(self, root_folder):
        ...
        if len(incorrect_rules) > 0:
            logging.exception(
                "The following names in the rules section don't have a definition: {}".format(incorrect_rules))
            raise TypeError(
                "The following names in the rules section don't have a definition: {}".format(incorrect_rules))
    ...
    def get_combination_tags(self, combination_list=[]):
        ...
        for rule in self.rl.rules_list:
            ...
            if len(match_all_list) == len(rule.all) and len(match_any_list) <= len(rule.any) and (
                    len(match_all_list) > 0 or len(match_any_list) > 0):
    ...
    def get_tags(self, api_directory, combination_list=[]):
        ...
        for rule in self.rl.rules_list:
            ...
            if len(match_all_dict) == len(rule.all) and len(match_any_dict) <= len(rule.any) and (
                    len(match_any_dict) > 0 or len(match_all_dict) > 0):
