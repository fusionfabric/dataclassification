...
class Definition:
    ...
    def __init__(
        ...
        definition: {
            "friendly_name": str,
            "description": str,
            "technical_names": list(str.__dict__),
        },
    ):
        ...
        self.technical_names_set = set(n.lower() for n in definition["technical_names"])
        ...
        if api_directory:
            if self.exception_names_set:
                for exception_names in self.exception_names_set:
                    (
                        api_directory_name,
                        exception_technical_name,
                    ) = exception_names.split(":")
                    if (
                        api_directory.lower()[:-1] == api_directory_name
                        and tech_name == exception_technical_name
                    ):
                        return True
...
class Rule:
    def __init__(
        ...
        root_folder,
        rule={
            "tags": list(str.__dict__),
            "all": list(str.__dict__),
            "any": list(str.__dict__),
        },
    ):
        self.tags = rule["tags"]
        self.all = rule["all"]
        ...
        if len(self.all) == 0 and len(self.any) == 0:
            logging.error(
                "A rule must have technical names in either 'all' or 'any' sections."
            )
            raise TypeError(
                "A rule must have technical names in either 'all' or 'any' sections."
            )
