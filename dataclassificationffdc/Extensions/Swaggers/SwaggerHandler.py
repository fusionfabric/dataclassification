    ...
    def write_file(self, sobj: SwaggerObj):
        ...
        with open(path, 'w') as file:
            ...
            if 'dataset-schema-OAS' in self.filenames:
                jsonObject = jsonObject.replace("x-legalTerms", "legalTerms").replace("x-id", "$id").replace(
                    "x-additionalProperties", "additionalProperties").replace("x-additionalItems",
                                                                              "additionalItems").replace(
    ...
    def get_json(self, root_directory, path):
        with open(path, encoding='utf-8-sig') as json_file:
            try:
                ...
                if 'info' in sobj.obj and self.maturity_status in sobj.obj['info'] and sobj.obj['info'][
                    self.maturity_status] == "DEPRECATED":
