...
class SwaggerHandler:
    ...
    def __init__(
        self, root_folder, maturity_status, parameter_exclusion_list, filenames
    ):
        ...
        self.oas2 = Oas2Handler(
            self.parameter_descriptions, self.parameter_exclusion_list
        )
        self.oas3 = Oas3Handler(
            self.parameter_descriptions, self.parameter_exclusion_list
        )
        try:
                        ...
                        for version in api["versions"]:
                            if (
                                ...
                                if "releaseDataClassifier" not in version:
                                    ...
                                    releaseDataClassifier = version[
                                        "releaseDataClassifier"
                                    ]
                                    if (
                                        releaseDataClassifier == "test"
                                        or releaseDataClassifier == "release"
                                    ):
                                        swaggerFilePath = (
                                            root_folder
                                            + "/"
                            ...
                            for files in os.walk(entry):
                                for file in files[2]:
                                    for filename in filenames:
                                        if (
                                            filename.lower() + ".json" == file.lower()
                                            and ".json" in file.lower()
                                            and ".log" not in file.lower()
                                            and "-classified" not in file.lower()
                                        ):
                                            self.json_list.append(
                                                "{}/{}/{}".format(
                                                    root_folder, entry.name, file
                                                )
                                            )
    ...
    def write_file(self, sobj: SwaggerObj):
        ...
        with open(path, "w") as file:
            ...
            if "dataset-schema-OAS" in self.filenames:
                jsonObject = (
                    jsonObject.replace("x-legalTerms", "legalTerms")
                    .replace("x-id", "$id")
                    .replace("x-additionalProperties", "additionalProperties")
                    .replace("x-additionalItems", "additionalItems")
                    .replace("/definitions/", "/properties/")
                )
    ...
    def get_json(self, root_directory, path):
        with open(path, encoding="utf-8-sig") as json_file:
            try:
                ...
                if (
                    "info" in sobj.obj
                    and self.maturity_status in sobj.obj["info"]
                    and sobj.obj["info"][self.maturity_status] == "DEPRECATED"
                ):
                    logging.error(
                        "This API is deprecated and cannot be considered by the tool {}".format(
                            path
                        )
                    )
                    raise ValueError(
                ...
                logging.error(
                    "Error reading or decoding the file {}: {}".format(path, exc)
                )
                raise ValueError(
                    "Error reading or decoding the file {}: {}".format(path, exc)
                )
    ...
    def get_extended_request_parameters_by_method(
        ...
        parameters_list = []
        if "swagger" in sobj.obj and sobj.obj["swagger"] == "2.0":
            parameters_list = self.oas2.get_extended_request_parameters_by_method(
                sobj, endpoint, method
            )
        else:
            parameters_list = self.oas3.get_extended_request_parameters_by_method(
                sobj, endpoint, method
            )
        return parameters_list
    ...
    def get_extended_response_parameters_by_method(
        ...
        parameters_list = []
        if "swagger" in sobj.obj and sobj.obj["swagger"] == "2.0":
            parameters_list = self.oas2.get_extended_response_parameters_by_method(
                sobj, endpoint, method
            )
        else:
            parameters_list = self.oas3.get_extended_response_parameters_by_method(
                sobj, endpoint, method
            )
        return parameters_list
