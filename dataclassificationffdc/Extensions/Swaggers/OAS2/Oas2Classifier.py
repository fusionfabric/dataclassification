            ...
            if 'parameters' in sobj.obj['paths'][endpoint][method]:
                for parameter in sobj.obj['paths'][endpoint][method]['parameters']:
                    if 'name' in parameter:
                        ...
                        if pname in tags:
                            sobj_new.obj['paths'][endpoint][method]['parameters'][param_counter][
                                self.classification_header] = self.__get__parameter_tags(tags, pname)
                    param_counter += 1
        return sobj_new
                            ...
                            if parameter == 'properties':
                                for item in (sobj.obj['paths'][endpoint][method]['responses'][response_code]['schema'][
                                    ...
                                    if 'type' in item[1] and item[0] in tags:
                                        sobj_new.obj['paths'][endpoint][method]['responses'][response_code]['schema'][
                                            self.classification_header] = self.__get__parameter_tags(tags, item[0])
                            elif parameter == 'type':
                                isfile = sobj_new.obj['paths'][endpoint][method]['responses'][response_code]['schema'][
                                    'type']
                                if isfile == 'file':
                                    sobj_new.obj['paths'][endpoint][method]['responses'][response_code]['schema'][
                                        self.classification_header] = self.__get__parameter_tags(tags, 'file')
        return sobj_new
                ...
                if 'properties' in sobj.obj['definitions'][definition]:
                    for p in sobj.obj['definitions'][definition]['properties']:
                        ...
                        if dname in tags:
                            sobj_new.obj['definitions'][definition]['properties'][p][
                                self.classification_header] = self.__get__parameter_tags(tags, dname)
                    ...
                    for pobjlist in sobj.obj['definitions'][definition]['allOf']:
                        if 'properties' in sobj_new.obj['definitions'][definition]['allOf'][list_counter]:
                            for p in sobj_new.obj['definitions'][definition]['allOf'][list_counter]['properties']:
                                dname = '{}/{}'.format(definition, p)
                                if dname in tags:
                                    sobj_new.obj['definitions'][definition]['allOf'][list_counter]['properties'][p][
                                        self.classification_header] = self.__get__parameter_tags(tags, dname)
                        list_counter += 1
                    ...
                    if 'properties' in sobj.obj['definitions'][definition]['items']:
                        for p in sobj.obj['definitions'][definition]['items']['properties']:
                            dname = '{}/{}'.format(definition, p)
                            if dname in tags:
                                sobj_new.obj['definitions'][definition]['items']['properties'][p][
                                    self.classification_header] = self.__get__parameter_tags(tags, dname)
                elif 'description' in sobj.obj['definitions'][definition] and definition in tags:
                    sobj_new.obj['definitions'][definition][self.classification_header] = self.__get__parameter_tags(
                        tags, definition)
        return sobj_new
    ...
    def classify_datasetdefinitions(self, sobj: SwaggerObj, tags: list):
        ...
        if 'definitions' in sobj_new.obj:
            for definition in sobj_new.obj['definitions']:
                self.classify_datasetdefinitions_recursive(sobj_new.obj['definitions'][definition], definition, tags)
        return sobj_new

    def classify_datasetdefinitions_recursive(self, sobj: SwaggerObj, definition: str, tags: list):
        if 'properties' in sobj:
            for p in sobj['properties']:
                ...
                if 'properties' in sobj['properties'][p]:
                    self.classify_datasetdefinitions_recursive(sobj['properties'][p], p, tags)
                elif 'items' in sobj['properties'][p]:
                    if sobj['properties'][p]['items']['type'] == "object":
                        self.classify_datasetdefinitions_recursive(sobj['properties'][p]['items'], p, tags)
                    else:
                        if dname in tags:
                            param_tag = self._get_matching_tag_for_header(tags, dname)
                            if "tag1" in param_tag.keys():
                                if len(param_tag["tag1"]) > 0:
                                    sobj['properties'][p][self.classification_header] = param_tag["tag1"]
                            if "tag2" in param_tag.keys():
                                if len(param_tag["tag2"]) > 0:
                                    sobj['properties'][p][self.encryption_header] = param_tag["tag2"]
                else:
                    if dname in tags:
                        param_tag = self._get_matching_tag_for_header(tags, dname)
                        if "tag1" in param_tag.keys():
            ...
            if sobj['items']['type'] == "object":
                self.classify_datasetdefinitions_recursive(sobj['items'], definition, tags)
            else:
                if definition:
                    dname = '{}/{}'.format(definition, p)
                else:
        ...
        if 'parameters' in sobj.obj:
            for parameter in sobj.obj['parameters']:
                if parameter in tags:
                    sobj_new.obj['parameters'][parameter][self.classification_header] = self.__get__parameter_tags(tags,
                                                                                                                   parameter)
        return sobj_new
