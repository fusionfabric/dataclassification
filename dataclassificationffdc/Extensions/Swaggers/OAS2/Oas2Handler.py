...
class Oas2Handler:
                ...
                if response_code in sobj.obj['paths'][endpoint][method]['responses']:
                    for parameter in sobj.obj['paths'][endpoint][method]['responses'][response_code]['schema']:
                        if parameter == 'properties':
                            for item in (sobj.obj['paths'][endpoint][method]['responses'][response_code]['schema'][
                                'properties']).items():
                                if 'items' in item[1]:
                                    ...
                                    print('Hnhandled response parameter in file {} and endpoint {}: {}',
                                          format(sobj.shortPath, endpoint, item))
                        elif parameter == 'items':
                            for item in (
                            sobj.obj['paths'][endpoint][method]['responses'][response_code]['schema']['items']).items():
                                if item[0] == '$ref':
                                    parameters_list[method].append(item[1])
                        elif parameter == '$ref':
                            parameters_list[method].append(
                                sobj.obj['paths'][endpoint][method]['responses'][response_code]['schema']['$ref'])
        return parameters_list
            ...
            for definition in sobj.obj['definitions']:
                if 'properties' in sobj.obj['definitions'][definition]:
                        ...
                        if 'items' in sobj.obj['definitions'][definition]['properties'][p]:
                            if '$ref' not in sobj.obj['definitions'][definition]['properties'][p]['items']:
                                if '$ref' not in sobj.obj['definitions'][definition]['properties'][p]:
                                    ...
                                    if {"file": sobj.shortPath,
                                        "description": sobj.obj['definitions'][definition]['properties'][p][
                                            "description"]} not in self.parameter_descriptions[dname]:
                                        self.parameter_descriptions[dname].append({"file": sobj.shortPath,
                                                                                   "description":
                                                                                       sobj.obj['definitions'][
                            ...
                            if '$ref' not in sobj.obj['definitions'][definition]['properties'][p]:
                                ...
                                if {"file": sobj.shortPath,
                                    "description": sobj.obj['definitions'][definition]['properties'][p][
                                        "description"]} not in self.parameter_descriptions[dname]:
                                    self.parameter_descriptions[dname].append({"file": sobj.shortPath, "description":
                                        sobj.obj['definitions'][definition]['properties'][p]["description"]})
                            ...
                            if '$ref' not in sobj.obj['definitions'][definition]['properties'][p]:
                                ...
                                if dname not in self.parameter_descriptions.keys():
                                    self.parameter_descriptions[dname] = []
                                if {"file": sobj.shortPath,
                                    "description": sobj.obj['definitions'][definition]['properties'][p]["title"]} not in \
                                        self.parameter_descriptions[dname]:
                                    self.parameter_descriptions[dname].append({"file": sobj.shortPath, "description":
                                        sobj.obj['definitions'][definition]['properties'][p]["title"]})
                                    ...
                                    if 'items' in pobjlist['properties'][p]:
                                        ...
                                        if '$ref' not in pobjlist['properties'][p]:
                                            ...
                                            if dname not in self.parameter_descriptions.keys():
                                                self.parameter_descriptions[dname] = []
                                            if {"file": sobj.shortPath,
                                                "description": pobjlist['properties'][p]['description']} not in \
                                                    self.parameter_descriptions[dname]:
                                                self.parameter_descriptions[dname].append({"file": sobj.shortPath,
                                                                                           "description":
                                                                                               pobjlist['properties'][
                                        ...
                                        if '$ref' not in pobjlist['properties'][p]:
                                            ...
                                            if dname not in self.parameter_descriptions.keys():
                                                self.parameter_descriptions[dname] = []
                                            if {"file": sobj.shortPath,
                                                "description": pobjlist['properties'][p]['title']} not in \
                                                    self.parameter_descriptions[dname]:
                                                self.parameter_descriptions[dname].append({"file": sobj.shortPath,
                                                                                           "description":
                                                                                               pobjlist['properties'][
                                        ...
                                        if dname not in self.parameter_descriptions.keys():
                                            self.parameter_descriptions[dname] = []
                                        if {"file": sobj.shortPath, "description": p} not in \
                                                self.parameter_descriptions[dname]:
                                            self.parameter_descriptions[dname].append(
                                                {"file": sobj.shortPath, "description": p})
                            ...
                            if 'description' in sobj.obj['definitions'][definition]['items']['properties'][p]:
                                if '$ref' not in sobj.obj['definitions'][definition]['items']['properties'][p]:
                                    ...
                                    if dname not in self.parameter_descriptions.keys():
                                        self.parameter_descriptions[dname] = []
                                    if {"file": sobj.shortPath,
                                        "description": sobj.obj['definitions'][definition]['items']['properties'][p][
                                            "description"]} not in self.parameter_descriptions[dname]:
                                        self.parameter_descriptions[dname].append({"file": sobj.shortPath,
                                                                                   "description":
                                                                                       sobj.obj['definitions'][
                                ...
                                if '$ref' not in sobj.obj['definitions'][definition]['items']['properties'][p]:
                                    ...
                                    if {"file": sobj.shortPath,
                                        "description": sobj.obj['definitions'][definition]['items']['properties'][p][
                                            "title"]} not in self.parameter_descriptions[dname]:
                                        self.parameter_descriptions[dname].append({"file": sobj.shortPath,
                                                                                   "description":
                                                                                       sobj.obj['definitions'][
                                                                                           definition]['items'][
                    ...
                    if '$ref' not in sobj.obj['definitions'][definition]:
                        ...
                        if {"file": sobj.shortPath,
                            "description": sobj.obj['definitions'][definition]['description']} not in \
                                self.parameter_descriptions[definition]:
                            self.parameter_descriptions[definition].append({"file": sobj.shortPath, "description":
                                sobj.obj['definitions'][definition]['description']})
                elif 'title' in sobj.obj['definitions'][definition]:
                    if '$ref' not in sobj.obj['definitions'][definition]:
                        definitions_list.append(definition)
                        if definition not in self.parameter_descriptions.keys():
                            self.parameter_descriptions[definition] = []
                        if {"file": sobj.shortPath, "description": sobj.obj['definitions'][definition]['title']} not in \
                                self.parameter_descriptions[definition]:
                            self.parameter_descriptions[definition].append(
                                {"file": sobj.shortPath, "description": sobj.obj['definitions'][definition]['title']})
                    ...
                    for ap in sobj.obj['definitions'][definition]['additionalProperties']:
                        if '$ref' not in ap:
                            raise ValueError(
                                "Unhandled 'addiotnal property' in definition name {} in file {}".format(ap,
                                                                                                         sobj.shortPath))
                elif 'pattern' in sobj.obj['definitions'][definition] and 'type' in sobj.obj['definitions'][definition]:
                    raise ValueError(
                        "No description for the definition name {} in file {}".format(definition, sobj.shortPath))
                elif '$ref' not in sobj.obj['definitions'][definition]:
                    definitions_list.append(definition)
                    if definition not in self.parameter_descriptions.keys():
                        self.parameter_descriptions[definition] = []
                    if {"file": sobj.shortPath, "description": definition} not in self.parameter_descriptions[
                        definition]:
                        self.parameter_descriptions[definition].append(
                            {"file": sobj.shortPath, "description": definition})
                else:
            ...
            for parameter in sobj.obj['parameters']:
                if parameter not in self.parameter_exclusion_list:
                    ...
                    if "description" in sobj.obj['parameters'][parameter]:
                        if {"file": sobj.shortPath,
                            "description": sobj.obj['parameters'][parameter]["description"]} not in \
                                self.parameter_descriptions[parameter]:
                            self.parameter_descriptions[parameter].append({"file": sobj.shortPath, "description":
                                sobj.obj['parameters'][parameter]["description"]})
                    else:
                        if {"file": sobj.shortPath, "description": parameter} not in self.parameter_descriptions[
                            parameter]:
                            self.parameter_descriptions[parameter].append(
                                {"file": sobj.shortPath, "description": parameter})
        return parameters
            ...
            for parameter in sobj.obj['paths'][endpoint][method]['parameters']:
                if '$ref' in parameter:
                    ...
                    if "description" in parameter:
                        if {"file": sobj.shortPath, "endpoint": endpoint,
                            "description": parameter['description']} not in self.parameter_descriptions[pname]:
                            self.parameter_descriptions[pname].append(
                                {"file": sobj.shortPath, "endpoint": endpoint, "description": parameter['description']})
                    else:
                        if {"file": sobj.shortPath, "endpoint": endpoint, "description": pname} not in \
                                self.parameter_descriptions[pname]:
                            self.parameter_descriptions[pname].append(
                                {"file": sobj.shortPath, "endpoint": endpoint, "description": pname})
                    ...
                    if 'schema' in sobj.obj['paths'][endpoint][method]['responses'][response_code]:
                        for parameter in sobj.obj['paths'][endpoint][method]['responses'][response_code]['schema']:
                            if parameter == 'properties':
                                for item in (sobj.obj['paths'][endpoint][method]['responses'][response_code]['schema'][
                                    ...
                                    if 'items' in item[1]:
                                        ...
                                        if pname not in self.parameter_descriptions.keys():
                                            self.parameter_descriptions[pname] = []
                                        if {"file": sobj.shortPath, "endpoint": endpoint,
                                            "description": item[1]["description"]} not in self.parameter_descriptions[
                                            pname]:
                                            self.parameter_descriptions[pname].append(
                                                {"file": sobj.shortPath, "endpoint": endpoint,
                                ...
                                isfile = sobj.obj['paths'][endpoint][method]['responses'][response_code]['schema'][
                                    'type']
                                if isfile == 'file':
                                    if 'file' not in self.parameter_descriptions.keys():
                                        self.parameter_descriptions['file'] = []
                                        if {"file": sobj.shortPath, "endpoint": endpoint, "description": 'file'} not in \
                                                self.parameter_descriptions['file']:
                                            self.parameter_descriptions['file'].append(
                                                {"file": sobj.shortPath, "endpoint": endpoint, "description": 'file'})
                                    parameters_list.append(
                                        sobj.obj['paths'][endpoint][method]['responses'][response_code]['schema'][
                                            'type'])
                                ...
                                if response_schema in sobj.obj[responses]:
                                    if 'schema' in sobj.obj[responses][response_schema]:
                                        if '$ref' in sobj.obj[responses][response_schema]['schema']:
                                            parameters_list.append(
                                                sobj.obj[responses][response_schema]['schema']['$ref'])
                    elif 'content' in sobj.obj['paths'][endpoint][method]['responses'][response_code]:
                        parameters_list.append(
                            sobj.obj['paths'][endpoint][method]['responses'][response_code]['content'][
                                'application/json']['schema']['$ref'])
        except:
            raise ValueError(
                'An issue occured with the structure of file {} and endpoint {}'.format(sobj.shortPath, endpoint))
        return parameters_list
                    ...
                    if 'properties' in sobj.obj['definitions'][definition]:
                        for p in sobj.obj['definitions'][definition]['properties']:
                            ...
                            if '$ref' in sobj.obj['definitions'][definition]['properties'][p]:
                                definitionsref[definition].append(
                                    sobj.obj['definitions'][definition]['properties'][p]['$ref'])
                            if 'items' in sobj.obj['definitions'][definition]['properties'][p]:
                                if '$ref' in sobj.obj['definitions'][definition]['properties'][p]['items']:
                                    definitionsref[definition].append(
                                        sobj.obj['definitions'][definition]['properties'][p]['items']['$ref'])
                    elif 'allOf' in sobj.obj['definitions'][definition]:
                        for pobjlist in sobj.obj['definitions'][definition]['allOf']:
                            for pobj in pobjlist:
                                if 'properties' in pobj:
                                    for p in pobjlist['properties']:
                                        ...
                                        if 'items' in pobjlist['properties'][p]:
                                            for item in pobjlist['properties'][p]['items']:
                                                if '$ref' in item:
                                                    definitionsref[definition].append(
                                                        pobjlist['properties'][p]['items']['$ref'])
                        ...
                        for ap in sobj.obj['definitions'][definition]['additionalProperties']:
                            if '$ref' in ap:
                                definitionsref[definition].append(
                                    sobj.obj['definitions'][definition]['additionalProperties']['$ref'])
                    elif 'items' in sobj.obj['definitions'][definition] and 'title' in sobj.obj['definitions'][
                        definition]:
                            ...
                            if '$ref' in i:
                                ...
                                for p in sobj.obj['definitions'][definition]['items']['properties']:
                                    if '$ref' in sobj.obj['definitions'][definition]['items']['properties'][p]:
                                        definitionsref[definition].append(
                                            sobj.obj['definitions'][definition]['items']['properties'][p]['$ref'])
    ...
    def get_dataset_properties(self, definition: str, sobj: SwaggerObj, definitions_list: [], path: str,
                               hasParent: bool):
            ...
            for p in sobj['properties']:
                ...
                if 'description' in sobj['properties'][p]:
                    if '$ref' not in sobj['properties'][p] and 'properties' not in sobj['properties'][p]:
                        ...
                        if dname not in self.parameter_descriptions.keys():
                            self.parameter_descriptions[dname] = []
                        if {"file": path, "description": sobj['properties'][p]["description"]} not in \
                                self.parameter_descriptions[dname]:
                            self.parameter_descriptions[dname].append(
                                {"file": path, "description": sobj['properties'][p]["description"]})
                elif 'title' in sobj['properties'][p]:
                    if '$ref' not in sobj['properties'][p] and 'properties' not in sobj['properties'][p]:
                        definitions_list.append(dname)
                        if dname not in self.parameter_descriptions.keys():
                            self.parameter_descriptions[dname] = []
                        if {"file": path, "description": sobj['properties'][p]["title"]} not in \
                                self.parameter_descriptions[dname]:
                            self.parameter_descriptions[dname].append(
                                {"file": path, "description": sobj['properties'][p]["title"]})
                else:
                    if '$ref' not in sobj['properties'][p] and 'properties' not in sobj['properties'][p]:
                        definitions_list.append(dname)
                        if dname not in self.parameter_descriptions.keys():
                ...
                if 'properties' in item:
                    for p in sobj['items']['properties']:
                        ...
                        if 'description' in sobj['items']['properties'][p]:
                            if '$ref' not in sobj['items']['properties'][p] and 'properties' not in \
                                    sobj['items']['properties'][p] and 'items' not in sobj['items']['properties'][p]:
                                definitions_list.append(dname)
                                if dname not in self.parameter_descriptions.keys():
                                    self.parameter_descriptions[dname] = []
                                if {"file": path, "description": sobj['items']['properties'][p]["description"]} not in \
                                        self.parameter_descriptions[dname]:
                                    self.parameter_descriptions[dname].append(
                                        {"file": path, "description": sobj['items']['properties'][p]["description"]})
                            ...
                            if '$ref' not in sobj['items']['properties'][p] and 'properties' not in \
                                ...
                                if dname not in self.parameter_descriptions.keys():
                                    self.parameter_descriptions[dname] = []
                                if {"file": path, "description": sobj['items']['properties'][p]["title"]} not in \
                                        self.parameter_descriptions[dname]:
                                    self.parameter_descriptions[dname].append(
                                        {"file": path, "description": sobj['items']['properties'][p]["title"]})
                        else:
                            if '$ref' not in sobj['items']['properties'][p] and 'properties' not in \
                                    sobj['items']['properties'][p] and 'items' not in sobj['items']['properties'][p]:
                                definitions_list.append(dname)
                ...
                if 'description' in sobj:
                    if '$ref' not in sobj and 'properties' not in sobj:
                        ...
                        if {"file": path, "description": sobj["description"]} not in self.parameter_descriptions[dname]:
                            self.parameter_descriptions[dname].append(
                                {"file": path, "description": sobj["description"]})
                elif 'title' in sobj:
                    if '$ref' not in sobj and 'properties' not in sobj:
                        definitions_list.append(dname)
                        if dname not in self.parameter_descriptions.keys():
