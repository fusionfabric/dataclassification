                ...
                if response_code in sobj.obj['paths'][endpoint][method]['responses']:
                    if 'content' in sobj.obj['paths'][endpoint][method]['responses'][response_code]:
                        for contentType in sobj.obj['paths'][endpoint][method]['responses'][response_code]['content']:
                            if 'schema' in sobj.obj['paths'][endpoint][method]['responses'][response_code]['content'][
                                contentType]:
                                for parameter in \
                                sobj.obj['paths'][endpoint][method]['responses'][response_code]['content'][contentType][
                                    'schema']:
                                    if parameter == 'properties':
                                        for item in (
                                        sobj.obj['paths'][endpoint][method]['responses'][response_code]['content'][
                                            contentType]['schema']['properties']).items():
                                            if 'items' in item[1]:
                                        ...
                                        for item in (
                                        sobj.obj['paths'][endpoint][method]['responses'][response_code]['content'][
                                            contentType]['schema']['items']).items():
                                            if item[0] == '$ref':
        ...
        if 'components' in sobj.obj:
            for schema in sobj.obj['components']['schemas']:
                if 'properties' in sobj.obj['components']['schemas'][schema]:
                        ...
                        if 'items' in sobj.obj['components']['schemas'][schema]['properties'][p]:
                            if '$ref' not in sobj.obj['components']['schemas'][schema]['properties'][p]['items']:
                                if '$ref' not in sobj.obj['components']['schemas'][schema]['properties'][p]:
                                    ...
                                    if {"file": sobj.shortPath,
                                        "description": sobj.obj['components']['schemas'][schema]['properties'][p][
                            ...
                            if '$ref' not in sobj.obj['components']['schemas'][schema]['properties'][p]:
                                ...
                                if {"file": sobj.shortPath,
                                    ...
                                    self.parameter_descriptions[dname].append({"file": sobj.shortPath, "description":
                                        sobj.obj['components']['schemas'][schema]['properties'][p]["description"]})
                            ...
                            if '$ref' not in sobj.obj['components']['schemas'][schema]['properties'][p]:
                                ...
                                if {"file": sobj.shortPath,
                                    ...
                                    self.parameter_descriptions[dname].append({"file": sobj.shortPath, "description":
                                        sobj.obj['components']['schemas'][schema]['properties'][p]["title"]})
                        ...
                        for p in sobj.obj['components']['schemas'][schema]['items']['properties']:
                            ...
                            if 'description' in sobj.obj['components']['schemas'][schema]['items']['properties'][p]:
                                if '$ref' not in sobj.obj['components']['schemas'][schema]['items']['properties'][p]:
                                    ...
                                    if dname not in self.parameter_descriptions.keys():
                                        self.parameter_descriptions[dname] = []
                                    if {"file": sobj.shortPath, "description":
                                        sobj.obj['components']['schemas'][schema]['items']['properties'][p][
                                            "description"]} not in self.parameter_descriptions[dname]:
                                        self.parameter_descriptions[dname].append({"file": sobj.shortPath,
                                                                                   "description":
                                                                                       sobj.obj['components'][
                    ...
                    if {"file": sobj.shortPath,
                        "description": sobj.obj['components']['schemas'][schema]['description']} not in \
                            self.parameter_descriptions[schema]:
                        self.parameter_descriptions[schema].append({"file": sobj.shortPath, "description":
                            sobj.obj['components']['schemas'][schema]['description']})
                elif 'title' in sobj.obj['components']['schemas'][schema]:
                    definitions_list.append(schema)
                    if schema not in self.parameter_descriptions.keys():
                        self.parameter_descriptions[schema] = []
                    if {"file": sobj.shortPath,
                        "description": sobj.obj['components']['schemas'][schema]['title']} not in \
                            self.parameter_descriptions[schema]:
                        self.parameter_descriptions[schema].append(
                            {"file": sobj.shortPath, "description": sobj.obj['components']['schemas'][schema]['title']})
                        ...
                        if '$ref' not in ap:
                            raise ValueError(
                                "Unhandled 'addiotnal property' in definition name {} in file {}".format(ap,
                                                                                                         sobj.shortPath))
                elif 'pattern' in sobj.obj['components']['schemas'][schema] and 'type' in \
                        sobj.obj['components']['schemas'][schema]:
                    raise ValueError(
                        "No description for the definition name {} in file {}".format(schema, sobj.shortPath))
                ...
                for parameter in sobj.obj['components']['parameters']:
                    if parameter not in self.parameter_exclusion_list:
                        ...
                        if 'description' in sobj.obj['components']['parameters'][parameter]:
                            if {"file": sobj.shortPath,
                                "description": sobj.obj['components']['parameters'][parameter]["description"]} not in \
                                    self.parameter_descriptions[parameter]:
                                self.parameter_descriptions[parameter].append({"file": sobj.shortPath, "description":
                                    sobj.obj['components']['parameters'][parameter]["description"]})
                        else:
            ...
            if 'content' in sobj.obj['paths'][endpoint][method]['requestBody']:
                for contentType in sobj.obj['paths'][endpoint][method]['requestBody']['content']:
                    if 'schema' in sobj.obj['paths'][endpoint][method]['requestBody']['content'][contentType]:
                        if '$ref' in sobj.obj['paths'][endpoint][method]['requestBody']['content'][contentType][
                            'schema']:
                            parameters_list.append(
                                sobj.obj['paths'][endpoint][method]['requestBody']['content'][contentType]['schema'][
                                    '$ref'])
        return parameters_list
                                        ...
                                        sobj.obj['paths'][endpoint][method]['responses'][response_code]['content'][
                                            ...
                                            if 'items' in item[1]:
                                                ...
                                                if pname not in self.parameter_descriptions.keys():
                                                    self.parameter_descriptions[pname] = []
                                                if {"file": sobj.shortPath, "endpoint": endpoint,
                                                    "description": item[1]["description"]} not in \
                                                        self.parameter_descriptions[pname]:
                                                    self.parameter_descriptions[pname].append(
                    ...
                    if 'properties' in sobj.obj['components']['schemas'][schema]:
                        for p in sobj.obj['components']['schemas'][schema]['properties']:
                            ...
                            if '$ref' in sobj.obj['components']['schemas'][schema]['properties'][p]:
                                deinitionref[schema].append(
                                    sobj.obj['components']['schemas'][schema]['properties'][p]['$ref'])
                            if 'items' in sobj.obj['components']['schemas'][schema]['properties'][p]:
                                if '$ref' in sobj.obj['components']['schemas'][schema]['properties'][p]['items']:
                                    deinitionref[schema].append(
                                        sobj.obj['components']['schemas'][schema]['properties'][p]['items']['$ref'])
                    elif 'allOf' in sobj.obj['components']['schemas'][schema]:
                        for pobjlist in sobj.obj['components']['schemas'][schema]['allOf']:
                            for pobj in pobjlist:
                                if 'properties' in pobj:
                        ...
                        for ap in sobj.obj['components']['schemas'][schema]['additionalProperties']:
                            if '$ref' in ap:
                                deinitionref[schema].append(
                                    sobj.obj['components']['schemas'][schema]['additionalProperties']['$ref'])
                    elif 'items' in sobj.obj['components']['schemas'][schema] and 'title' in \
                            sobj.obj['components']['schemas'][schema]:
                            ...
                            if '$ref' in i:
                                ...
                                for p in sobj.obj['components']['schemas'][schema]['items']['properties']:
                                    if '$ref' in sobj.obj['components']['schemas'][schema]['items']['properties'][p]:
                                        deinitionref[schema].append(
                                            sobj.obj['components']['schemas'][schema]['items']['properties'][p]['$ref'])
