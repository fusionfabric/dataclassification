            ...
            for response_code in covered_responses:
                if response_code in sobj.obj["paths"][endpoint][method]["responses"]:
                    if (
                        ...
                        for contentType in sobj.obj["paths"][endpoint][method][
                            "responses"
                            ...
                            if (
                                ...
                                for parameter in sobj.obj["paths"][endpoint][method][
                                    "responses"
                                ][response_code]["content"][contentType]["schema"]:
                                    if parameter == "properties":
                                        for item in (
                                            sobj.obj["paths"][endpoint][method][
                                                "responses"
                                            ][response_code]["content"][contentType][
                                                "schema"
                                            ][
                                                "properties"
                                            ]
            ...
            for schema in sobj.obj["components"]["schemas"]:
                if "properties" in sobj.obj["components"]["schemas"][schema]:
                    for p in sobj.obj["components"]["schemas"][schema]["properties"]:
                        ...
                        if (
                            "items"
                            in sobj.obj["components"]["schemas"][schema]["properties"][
                                p
                            ]
                        ):
                            if (
                                "$ref"
                                not in sobj.obj["components"]["schemas"][schema][
                                    "properties"
                                ][p]["items"]
                            ):
                                if (
                                    ...
                                    if {
                                        "file": sobj.shortPath,
                                        "description": sobj.obj["components"][
                                            "schemas"
                            ...
                            if (
                                ...
                                if {
                                    "file": sobj.shortPath,
                                    "description": sobj.obj["components"]["schemas"][
                                        schema
                            ...
                            if "properties" in pobj:
                                for p in pobjlist["properties"]:
                                    ...
                                    if "items" in pobjlist["properties"][p]:
                                        ...
                                        if {
                                            "file": sobj.shortPath,
                                            "description": pobjlist["properties"][p][
                                                "description"
                                            ],
                                        } not in self.parameter_descriptions[dname]:
                                            self.parameter_descriptions[dname].append(
                    ...
                    if (
                        "properties"
                        in sobj.obj["components"]["schemas"][schema]["items"]
                    ):
                        for p in sobj.obj["components"]["schemas"][schema]["items"][
                            "properties"
                        ]:
                            dname = "{}/{}".format(schema, p)
                            if (
                                ...
                                if (
                                    ...
                                    if {
                                        ...
                                        "description": sobj.obj["components"][
                                            "schemas"
                                        ][schema]["items"]["properties"][p]["title"],
                                    } not in self.parameter_descriptions[dname]:
                                        self.parameter_descriptions[dname].append(
                                            {
                                                "file": sobj.shortPath,
                    ...
                    if {
                        "file": sobj.shortPath,
                        "description": sobj.obj["components"]["schemas"][schema][
                            "description"
                        ],
                    } not in self.parameter_descriptions[schema]:
                    ...
                    "additionalProperties" in sobj.obj["components"]["schemas"][schema]
                ):
                    for ap in sobj.obj["components"]["schemas"][schema][
                    ...
                    "pattern" in sobj.obj["components"]["schemas"][schema]
                    and "type" in sobj.obj["components"]["schemas"][schema]
                ):
                    raise ValueError(
                        "No description for the definition name {} in file {}".format(
                            schema, sobj.shortPath
                        )
                    )
                    ...
                    if {
                        ...
                        self.parameter_descriptions[schema].append(
                            {"file": sobj.shortPath, "description": schema}
                        )
                else:
                    print(
            ...
            if "parameters" in sobj.obj["components"]:
                for parameter in sobj.obj["components"]["parameters"]:
                    if parameter not in self.parameter_exclusion_list:
                        ...
                        if (
                            "description"
                            in sobj.obj["components"]["parameters"][parameter]
                        ):
                            if {
                                "file": sobj.shortPath,
                                "description": sobj.obj["components"]["parameters"][
                                    parameter
                                ]["description"],
            ...
            if "parameters" in sobj.obj["components"]:
                for parameter in sobj.obj["components"]["parameters"]:
                    if "schema" in sobj.obj["components"]["parameters"][parameter]:
                        if (
                            "$ref"
                            in sobj.obj["components"]["parameters"][parameter]["schema"]
                        ):
                            parameters.append(
                                sobj.obj["components"]["parameters"][parameter][
                                    "schema"
                                ]["$ref"]
                            )
        return parameters
    ...
    def get_extended_request_parameters_by_method(
        ...
        if "requestBody" in sobj.obj["paths"][endpoint][method]:
            if "content" in sobj.obj["paths"][endpoint][method]["requestBody"]:
                for contentType in sobj.obj["paths"][endpoint][method]["requestBody"][
                    "content"
                ]:
                    if (
                        "schema"
                        in sobj.obj["paths"][endpoint][method]["requestBody"][
                            "content"
                        ][contentType]
                    ):
                        if (
                            ...
                            parameters_list.append(
                                sobj.obj["paths"][endpoint][method]["requestBody"][
                                    "content"
                                        ...
                                        for item in (
                                            ...
                                            if "items" in item[1]:
                                                ...
                                                if (
                                                    ...
                                                    not in self.parameter_descriptions[
                                                        pname
                                                    ]
                                                ):
                                                    self.parameter_descriptions[
        ...
        if "components" in sobj.obj:
            if "schemas" in sobj.obj["components"]:
                for schema in sobj.obj["components"]["schemas"]:
                    deinitionref[schema] = []
                    if "properties" in sobj.obj["components"]["schemas"][schema]:
                        for p in sobj.obj["components"]["schemas"][schema][
                            "properties"
                        ]:
                            deinitionref[schema].append(p)
                            if (
                                "$ref"
                                in sobj.obj["components"]["schemas"][schema][
                                    "properties"
                                ][p]
                            ):
                        ...
                        for pobjlist in sobj.obj["components"]["schemas"][schema][
                            "allOf"
                        ]:
                            for pobj in pobjlist:
                                if "properties" in pobj:
                        ...
                        "items" in sobj.obj["components"]["schemas"][schema]
                        and "title" in sobj.obj["components"]["schemas"][schema]
                    ):
                        for i in sobj.obj["components"]["schemas"][schema]["items"]:
                            if "$ref" in i:
                                deinitionref[schema].append(
                                    sobj.obj["components"]["schemas"][schema]["items"][
                                        "$ref"
                                    ]
                                )
