                                ...
                                if 'schema' in \
                                    ...
                                    sobj.obj['paths'][endpoint][method]['responses'][response_code]['content'][
                                        ...
                                        if parameter == 'properties':
                                            for item in (
                                            sobj.obj['paths'][endpoint][method]['responses'][response_code]['content'][
                                                contentType]['schema'][parameter]).items():
                                                if 'type' in item[1] and item[0] in tags:
                                                    sobj_new.obj['paths'][endpoint][method]['responses'][response_code][
                                                        'content'][contentType]['schema'][
                                                        self.classification_header] = self.__get__parameter_tags(tags,
                                                                                                                 item[
                                                                                                                     0])
        return sobj_new
                ...
                for schema in sobj.obj['components']['schemas']:
                    if 'properties' in sobj.obj['components']['schemas'][schema]:
                        for p in sobj.obj['components']['schemas'][schema]['properties']:
                            ...
                            if dname in tags:
                                sobj_new.obj['components']['schemas'][schema]['properties'][p][
                        ...
                        for pobjlist in sobj.obj['components']['schemas'][schema]['allOf']:
                            if 'properties' in sobj_new.obj['components']['schemas'][schema]['allOf'][list_counter]:
                                for p in sobj_new.obj['components']['schemas'][schema]['allOf'][list_counter][
                                    'properties']:
                                    dname = '{}/{}'.format(schema, p)
                                    if dname in tags:
                                        sobj_new.obj['components']['schemas'][schema]['allOf'][list_counter][
                                            'properties'][p][self.classification_header] = self.__get__parameter_tags(
                                            tags, dname)
                            list_counter += 1
                        ...
                        for pobjlist in sobj.obj['components']['schemas'][schema]['anyOf']:
                            if 'properties' in sobj_new.obj['components']['schemas'][schema]['anyOf'][list_counter]:
                                for p in sobj_new.obj['components']['schemas'][schema]['anyOf'][list_counter][
                                    'properties']:
                                    dname = '{}/{}'.format(schema, p)
                                    if dname in tags:
                                        sobj_new.obj['components']['schemas'][schema]['anyOf'][list_counter][
                                            'properties'][p][self.classification_header] = self.__get__parameter_tags(
                                            tags, dname)
                            list_counter += 1
                        ...
                        for pobjlist in sobj.obj['components']['schemas'][schema]['oneOf']:
                            if 'properties' in sobj_new.obj['components']['schemas'][schema]['oneOf'][list_counter]:
                                for p in sobj_new.obj['components']['schemas'][schema]['oneOf'][list_counter][
                                    'properties']:
                                    dname = '{}/{}'.format(schema, p)
                                    if dname in tags:
                                        sobj_new.obj['components']['schemas'][schema]['oneOf'][list_counter][
                                            'properties'][p][self.classification_header] = self.__get__parameter_tags(
                                            tags, dname)
                            list_counter += 1
                        ...
                        if 'properties' in sobj.obj['components']['schemas'][schema]['items']:
                            for p in sobj.obj['components']['schemas'][schema]['items']['properties']:
                                dname = '{}/{}'.format(schema, p)
                                if dname in tags:
                                    sobj_new.obj['components']['schemas'][schema]['items']['properties'][p][
                                        self.classification_header] = self.__get__parameter_tags(tags, dname)
                    elif 'description' in sobj.obj['components']['schemas'][schema] and schema in tags:
                        sobj_new.obj['components']['schemas'][schema][
                            self.classification_header] = self.__get__parameter_tags(tags, schema)
            ...
            if 'parameters' in sobj.obj['components']:
                for parameter in sobj.obj['components']['parameters']:
                    if parameter in tags:
                        sobj_new.obj['components']['parameters'][parameter][
                            self.classification_header] = self.__get__parameter_tags(tags, parameter)
        return sobj_new
