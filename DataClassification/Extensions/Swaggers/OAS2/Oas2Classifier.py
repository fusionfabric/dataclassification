                    ...
                    if 'schema' in sobj.obj['paths'][endpoint][method]['responses'][response_code]:
                        for parameter in sobj.obj['paths'][endpoint][method]['responses'][response_code]['schema']:
                            if parameter == 'properties':
                                ...
                                if isfile == 'file':
                                    sobj_new.obj['paths'][endpoint][method]['responses'][response_code]['schema'][self.classification_header] = self.__get__parameter_tags(tags, 'file')
        return sobj_new
                ...
                if 'properties' in sobj.obj['definitions'][definition]:
                    ...
                    for pobjlist in sobj.obj['definitions'][definition]['allOf']: #[1]['properties']:
                        if 'properties' in sobj_new.obj['definitions'][definition]['allOf'][list_counter]:
                            for p in sobj_new.obj['definitions'][definition]['allOf'][list_counter]['properties']:
                                dname = '{}/{}'.format(definition,p)
                                if dname in tags:
                                    sobj_new.obj['definitions'][definition]['allOf'][list_counter]['properties'][p][self.classification_header] = self.__get__parameter_tags(tags,dname)
                        list_counter += 1
                    ...
                    if 'properties' in sobj.obj['definitions'][definition]['items']:
                        for p in sobj.obj['definitions'][definition]['items']['properties']:
                            dname = '{}/{}'.format(definition,p)
                            if dname in tags:
                                    sobj_new.obj['definitions'][definition]['items']['properties'][p][self.classification_header] = self.__get__parameter_tags(tags,dname)
                elif 'description' in sobj.obj['definitions'][definition] and definition in tags:
                    sobj_new.obj['definitions'][definition][self.classification_header] = self.__get__parameter_tags(tags,definition)
        return sobj_new
        ...
        if 'parameters' in sobj.obj:
            for parameter in sobj.obj['parameters']:
                if parameter in tags:
                    sobj_new.obj['parameters'][parameter][self.classification_header] = self.__get__parameter_tags(tags,parameter)
        return sobj_new
