                        ...
                        for contentType in sobj.obj["paths"][endpoint][method][
                            ...
                            if content_types.__contains__(contentType):
                                if (
                                    ...
                                    for parameter in sobj.obj["paths"][endpoint][
                                        method
                ...
                for schema in sobj.obj["components"]["schemas"]:
                    if "properties" in sobj.obj["components"]["schemas"][schema]:
                        ...
                        for pobjlist in sobj.obj["components"]["schemas"][schema][
                            ...
                            if (
                                "properties"
                                in sobj_new.obj["components"]["schemas"][schema][
                                    "allOf"
                                ][list_counter]
                            ):
                                for p in sobj_new.obj["components"]["schemas"][schema][
                                    "allOf"
                                ][list_counter]["properties"]:
                                    dname = "{}/{}".format(schema, p)
                                    if dname in tags:
                                        sobj_new.obj["components"]["schemas"][schema][
                                            "allOf"
                                        ][list_counter]["properties"][p][
                                            self.classification_header
                                        ] = self.__get__parameter_tags(
                                            tags, dname
                                        )
                        ...
                        "description" in sobj.obj["components"]["schemas"][schema]
                        and schema in tags
                    ):
                        sobj_new.obj["components"]["schemas"][schema][
                            self.classification_header
                        ] = self.__get__parameter_tags(tags, schema)
                    elif (
                        "title" in sobj.obj["components"]["schemas"][schema]
                        and schema in tags
                    ):
                        sobj_new.obj["components"]["schemas"][schema][
                            self.classification_header
                        ...
                        "$ref" not in sobj.obj["components"]["schemas"][schema]
                        and schema in tags
                    ):
                        sobj_new.obj["components"]["schemas"][schema][
                            self.classification_header
