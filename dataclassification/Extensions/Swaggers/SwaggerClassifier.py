...
class SwaggerClassifier:
    ...
    def __init__(
        ...
        logging.basicConfig(
            filename=swagger_folder_path + "/data-classification.log",
            format="%(asctime)s - %(levelname)s - %(message)s",
            level=logging.INFO,
        )
        self.classification_header = classification_header
        self.encryption_header = encryption_header
        self.sh = SwaggerHandler(
            swagger_folder_path, maturity_status, parameter_exclusion_list, filenames
        )
        self.ce = ClassificationEngine(swagger_folder_path, rules_file_path)
        self.filenames = filenames
        self.oas2Classifier = Oas2Classifier(
            classification_header, encryption_header, tags_list
        )
    ...
    def classify_request_parameters(
        self, api_directory, sobj: SwaggerObj, endpoint: str
    ):
        tags = self.__get_all_parameters_in_swagger(api_directory, sobj)
        if "swagger" in sobj.obj and sobj.obj["swagger"] == "2.0":
            sobj_new = self.oas2Classifier.classify_request_parameters(
                sobj, endpoint, tags
            )
        else:
            sobj_new = self.oas3Classifier.classify_request_parameters(
                sobj, endpoint, tags
            )
        return sobj_new

    def classify_response_parameters(
        self, api_directory, sobj: SwaggerObj, endpoint: str
    ):
        tags = self.__get_all_parameters_in_swagger(api_directory, sobj)
        if "swagger" in sobj.obj and sobj.obj["swagger"] == "2.0":
            sobj_new = self.oas2Classifier.classify_response_parameters(
                sobj, endpoint, tags
            )
        else:
            sobj_new = self.oas3Classifier.classify_response_parameters(
                sobj, endpoint, tags
            )
        return sobj_new
    ...
    def classify_API_and_endpoints(self, api_directory, sobj: SwaggerObj):
        ...
        for endpoint in sobj.obj["paths"]:
            for method in sobj.obj["paths"][endpoint]:
                endpoint_tags = self.__get_endpoint_tags(
                    api_directory, sobj, endpoint, method
                )
    ...
    def __get_all_parameters_in_swagger(self, api_directory, sobj: SwaggerObj):
        ...
        for endpoint in self.sh.get_endpoints(sobj):
            ...
            response_parameters = self.sh.get_extended_response_parameters(
                sobj, endpoint
            )
            all_parameters = (
                all_parameters
    ...
    def __get_endpoint_tags(
        self, api_directory, sobj: SwaggerObj, endpoint: str, method: str
    ):
        parameters = []
        p = self.sh.get_parameters(sobj)
        d = self.sh.get_definitions(sobj)
        param_refernced = self.sh.get_parameter_references(sobj)
        request_parameters = self.sh.get_extended_request_parameters_by_method(
            sobj, endpoint, method
        )
        response_parameters = self.sh.get_extended_response_parameters_by_method(
            sobj, endpoint, method
        )
        ...
        for rq in all_params:
            if "#/" in rq:
                if "swagger" in sobj.obj and sobj.obj["swagger"] == "2.0":
                    rq_unreferenced = rq.replace("#/parameters/", "").replace(
                        "#/definitions/", ""
                    )
                else:
                    rq_unreferenced = rq.replace("#/parameters/", "").replace(
                ...
                if rq_unreferenced in all_referenced:
                    ...
                    rq_unreferenced += "/"
                    ref_params = [
                        i for i in all_referenced if i.startswith(rq_unreferenced)
                    ]
                ...
                for item in reflist:
                    if item in all_referenced:
                        ...
                        ref_params += [
                            i for i in all_referenced if i.startswith(item + "/")
                        ]
                        parameters += ref_params
            else:
        ...
        for tag_name in tags:
            if tag_name in self.tags_type_list["tag1"]:
                if "tag1" not in tags_by_type.keys():
                    ...
                    tags_by_type["tag1"].append(tag_name)
            if (
                "tag2" in self.tags_type_list
                and tag_name in self.tags_type_list["tag2"]
            ):
