...
class Oas2Handler:
        ...
        for method in sobj.obj["paths"][endpoint]:
            ...
            for response_code in covered_responses:
                if response_code in sobj.obj["paths"][endpoint][method]["responses"]:
                    for parameter in sobj.obj["paths"][endpoint][method]["responses"][
                        response_code
                    ]["schema"]:
                        if parameter == "properties":
                            for item in (
                                ...
                                if "items" in item[1]:
                                    ...
                                    print(
                                        "Hnhandled response parameter in file {} and endpoint {}: {}",
            ...
            for definition in sobj.obj["definitions"]:
                if "properties" in sobj.obj["definitions"][definition]:
                    for p in sobj.obj["definitions"][definition]["properties"]:
                        ...
                        if (
                            "items"
                            in sobj.obj["definitions"][definition]["properties"][p]
                        ):
                            if (
                                "$ref"
                                not in sobj.obj["definitions"][definition][
                                    "properties"
                                ][p]["items"]
                            ):
                                if (
                                    ...
                                    if {
                                        "file": sobj.shortPath,
                                        "description": sobj.obj["definitions"][
                                            definition
                                        ]["properties"][p]["description"],
                                    } not in self.parameter_descriptions[dname]:
                            ...
                            if (
                                ...
                                if {
                                    "file": sobj.shortPath,
                                    "description": sobj.obj["definitions"][definition][
                                        "properties"
                                    ][p]["description"],
                                } not in self.parameter_descriptions[dname]:
                                    self.parameter_descriptions[dname].append(
                            ...
                            "$ref"
                            not in sobj.obj["definitions"][definition]["properties"][p]
                        ):
                            definitions_list.append(dname)
                            if dname not in self.parameter_descriptions.keys():
                                self.parameter_descriptions[dname] = []
                                ...
                                for p in pobjlist["properties"]:
                                    ...
                                    if "items" in pobjlist["properties"][p]:
                                        ...
                                        if "$ref" not in pobjlist["properties"][p]:
                                            ...
                                            if (
                                                dname
                                                not in self.parameter_descriptions.keys()
                                            ):
                                                self.parameter_descriptions[dname] = []
                                            if {
                                                "file": sobj.shortPath,
                                                "description": pobjlist["properties"][
                                                    p
                                                ]["description"],
                                            } not in self.parameter_descriptions[dname]:
                    ...
                    if "properties" in sobj.obj["definitions"][definition]["items"]:
                        for p in sobj.obj["definitions"][definition]["items"][
                            ...
                            if (
                                "description"
                                in sobj.obj["definitions"][definition]["items"][
                                    "properties"
                                ][p]
                            ):
                                if (
                                    "$ref"
                                    not in sobj.obj["definitions"][definition]["items"][
                                        "properties"
                                    ][p]
                                ):
                                    definitions_list.append(dname)
                                    ...
                                    if {
                                        ...
                                        "description": sobj.obj["definitions"][
                                            definition
                                        ]["items"]["properties"][p]["description"],
                                    } not in self.parameter_descriptions[dname]:
                                        self.parameter_descriptions[dname].append(
                                            {
                                                "file": sobj.shortPath,
                    ...
                    if "$ref" not in sobj.obj["definitions"][definition]:
                        ...
                        } not in self.parameter_descriptions[definition]:
                    ...
                    if "$ref" not in sobj.obj["definitions"][definition]:
                        ...
                        if {
                            "file": sobj.shortPath,
                            "description": sobj.obj["definitions"][definition]["title"],
                        } not in self.parameter_descriptions[definition]:
                            self.parameter_descriptions[definition].append(
                                {
                                    "file": sobj.shortPath,
                    ...
                    for ap in sobj.obj["definitions"][definition][
                        "additionalProperties"
                    ]:
                        if "$ref" not in ap:
                            raise ValueError(
                                "Unhandled 'addiotnal property' in definition name {} in file {}".format(
                                    ap, sobj.shortPath
                                )
                            )
                elif (
                    "pattern" in sobj.obj["definitions"][definition]
                    and "type" in sobj.obj["definitions"][definition]
                ):
                    raise ValueError(
                        "No description for the definition name {} in file {}".format(
                            definition, sobj.shortPath
                        )
                    )
                    ...
                    print(
                        "Unparsable definition {} in file {}".format(
                            definition, sobj.shortPath
                        )
                    )
            ...
            for parameter in sobj.obj["parameters"]:
                if parameter not in self.parameter_exclusion_list:
                    ...
                    if "description" in sobj.obj["parameters"][parameter]:
                        if {
                            "file": sobj.shortPath,
                            "description": sobj.obj["parameters"][parameter][
                                "description"
                            ],
                        } not in self.parameter_descriptions[parameter]:
            ...
            for parameter in sobj.obj["parameters"]:
                if "schema" in sobj.obj["parameters"][parameter]:
                    if "$ref" in sobj.obj["parameters"][parameter]["schema"]:
                        parameters.append(
                            sobj.obj["parameters"][parameter]["schema"]["$ref"]
                        )
        return parameters
    ...
    def get_extended_request_parameters(self, sobj: SwaggerObj, endpoint: str):
        parameters_list = []
        for method in sobj.obj["paths"][endpoint]:
            parameters_list += self.get_extended_request_parameters_by_method(
                sobj, endpoint, method
            )
        return parameters_list

    def get_extended_request_parameters_by_method(
        self, sobj: SwaggerObj, endpoint: str, method: str
    ):
        parameters_list = []
            ...
            for parameter in sobj.obj["paths"][endpoint][method]["parameters"]:
                if "$ref" in parameter:
                    ...
                    if "description" in parameter:
                        ...
                        } not in self.parameter_descriptions[pname]:
    ...
    def get_extended_response_parameters(self, sobj: SwaggerObj, endpoint: str):
        parameters_list = []
        for method in sobj.obj["paths"][endpoint]:
            parameters_list += self.get_extended_response_parameters_by_method(
                sobj, endpoint, method
            )
        return parameters_list

    def get_extended_response_parameters_by_method(
        self, sobj: SwaggerObj, endpoint: str, method: str
    ):
        parameters_list = []
        covered_responses = ["200", "201", "202"]
                    ...
                    if (
                        ...
                        for parameter in sobj.obj["paths"][endpoint][method][
                            ...
                            if parameter == "properties":
                                ...
                                isfile = sobj.obj["paths"][endpoint][method][
                                    "responses"
                                ...
                                if isfile == "file":
                                    if "file" not in self.parameter_descriptions.keys():
                                        ...
                                        if {
                                            ...
                                            "endpoint": endpoint,
                                            "description": "file",
                                        } not in self.parameter_descriptions["file"]:
                                    ...
                                    parameters_list.append(
                                        sobj.obj["paths"][endpoint][method][
                                            "responses"
                                        ][response_code]["schema"]["type"]
                                    )
                    elif (
                        ...
                        ref = sobj.obj["paths"][endpoint][method]["responses"][
                            response_code
                        ]["$ref"]
                        items = ref.split("/")
                        if len(items) > 1:
                            ...
                            if responses in sobj.obj:
                                if response_schema in sobj.obj[responses]:
                                    if "schema" in sobj.obj[responses][response_schema]:
                                        if (
                                            "$ref"
                                            in sobj.obj[responses][response_schema][
                                                "schema"
                                            ]
                                        ):
                                            parameters_list.append(
                                                sobj.obj[responses][response_schema][
                                                    "schema"
                                                ]["$ref"]
                                            )
                    elif (
            ...
            if "definitions" in sobj.obj:
                for definition in sobj.obj["definitions"]:
                    ...
                    if "properties" in sobj.obj["definitions"][definition]:
                        for p in sobj.obj["definitions"][definition]["properties"]:
                            ...
                            if (
                                ...
                                definitionsref[definition].append(
                                    sobj.obj["definitions"][definition]["properties"][
                                        p
                                    ]["$ref"]
                                )
                            if (
                                ...
                                if (
                                    "$ref"
                                    in sobj.obj["definitions"][definition][
                                        "properties"
                                    ][p]["items"]
                                ):
                                    definitionsref[definition].append(
                                        sobj.obj["definitions"][definition][
                                            "properties"
                                        ][p]["items"]["$ref"]
                                    )
                    elif "allOf" in sobj.obj["definitions"][definition]:
                            ...
                            for pobj in pobjlist:
                                if "properties" in pobj:
                                    for p in pobjlist["properties"]:
                                        definitionsref[definition].append(p)
                                        if "items" in pobjlist["properties"][p]:
                                            for item in pobjlist["properties"][p][
                                                "items"
                                            ]:
                                                if "$ref" in item:
                                                    definitionsref[definition].append(
                        ...
                        "items" in sobj.obj["definitions"][definition]
                        and "title" in sobj.obj["definitions"][definition]
                    ):
                        for i in sobj.obj["definitions"][definition]["items"]:
                            if "$ref" in i:
                                definitionsref[definition].append(
                                    sobj.obj["definitions"][definition]["items"]["$ref"]
                                )
        ...
        if "definitions" in sobj.obj:
            for definition in sobj.obj["definitions"]:
                ...
                self.get_dataset_properties(
                    definition, sobj_new, definitions_list, sobj.shortPath, False
                )
        return definitions_list
        ...
        if "properties" in sobj:
            for p in sobj["properties"]:
                ...
                if "description" in sobj["properties"][p]:
                    if (
                        "$ref" not in sobj["properties"][p]
                        and "properties" not in sobj["properties"][p]
                    ):
                        definitions_list.append(dname)
                        if dname not in self.parameter_descriptions.keys():
                            self.parameter_descriptions[dname] = []
                        if {
                            "file": path,
                            "description": sobj["properties"][p]["description"],
                        } not in self.parameter_descriptions[dname]:
                            self.parameter_descriptions[dname].append(
                                {
                                    "file": path,
                    ...
                    if (
                        ...
                        if {
                            "file": path,
                            "description": sobj["properties"][p]["title"],
                        } not in self.parameter_descriptions[dname]:
                            self.parameter_descriptions[dname].append(
                                {
                                    "file": path,
                ...
                self.get_dataset_properties(
                    p, sobj["properties"][p], definitions_list, path, True
                )
        elif "items" in sobj:
                    ...
                    for p in sobj["items"]["properties"]:
                        ...
                        if "description" in sobj["items"]["properties"][p]:
                            if (
                                ...
                                and "items" not in sobj["items"]["properties"][p]
                            ):
                                definitions_list.append(dname)
                                if dname not in self.parameter_descriptions.keys():
                                    self.parameter_descriptions[dname] = []
                                if {
                                    "file": path,
                                    "description": sobj["items"]["properties"][p][
                                        "description"
                                    ],
                                } not in self.parameter_descriptions[dname]:
                                    self.parameter_descriptions[dname].append(
                                        {
                                            "file": path,
                                            "description": sobj["items"]["properties"][
                                                p
                                            ]["description"],
                                        }
                                    )
            ...
            if hasParent is False:
                ...
                if "description" in sobj:
                    ...
                    if "$ref" not in sobj and "properties" not in sobj:
                        ...
                        if {
                            "file": path,
                            "description": sobj["title"],
                        } not in self.parameter_descriptions[dname]:
                            self.parameter_descriptions[dname].append(
                                {"file": path, "description": sobj["title"]}
                            )
                else:
                    if "$ref" not in sobj and "properties" not in sobj:
                        definitions_list.append(dname)
