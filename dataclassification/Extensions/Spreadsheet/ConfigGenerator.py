...
class JSONGenerator:
    ...
    def __init__(
        ...
        filename=os.path.join(
            os.path.dirname(__file__), "..", "..", "Config/Dictionary.xlsx"
        ),
    ):
        for handler in logging.root.handlers[:]:
            logging.root.removeHandler(handler)
        logging.basicConfig(
            filename=product_directory + "/data-classification.log",
    ...
    def generate_json_files(
        self,
        path=os.path.join(os.path.dirname(__file__), "..", "..", "Config/rules.json"),
    ):
        logging.info("Fetching definitions and rules for rules.json file")
        definitions_obj = json.dumps(
            {"definitions": self.definitions_list, "rules": self.get_rules()}
        )
        try:
    ...
    def get_definitions(self):
        ...
        try:
            ...
            for i in range(2, field_types_sheet.max_row + 1):
                if (
                    field_types_sheet.cell(row=i, column=1).value == "None"
                    or field_types_sheet.cell(row=i, column=2).value == "None"
                ):
                    raise ValueError(
                        "Error in row #{}: Field Types and Descriptions cannot be empty".format(
                            i + 1
                        )
                    )
                definition = {
                    ...
                    "technical_names": self.__get_technical_names_by_friendly_name(
                        field_types_sheet.cell(row=i, column=1).value
                    ),
                    "Exception_API_and_Values": self.__get_exception_names_by_friendly_name(
                        field_types_sheet.cell(row=i, column=1).value
                    ),
                }
                ...
                for c in column_indexes["tags"]:
                    ...
                    if rules_sheet.cell(row=i, column=c).value is not None:
                        ...
                        if tagn not in self.tag_list.keys():
                            self.tag_list[tagn] = []
                        self.tag_list[tagn].append(
                            rules_sheet.cell(row=i, column=c).value
                        )

                if (len(rule["any"]) > 0 or len(rule["all"]) > 0) and len(
                    rule["tags"]
                ) > 0:
                    rules_list.append(rule)
                elif len(rule["any"]) > 0 or len(rule["all"]) > 0:
                    raise ValueError(
                        "Error in row #{}: Tags must be assigned to each rule".format(
                            i + 1
                        )
                    )
...
class SpreadsheetGenerator:
    ...
    def __init__(
        self,
        root_folder,
        swagger_classifier: SwaggerClassifier,
        ...
        self.title_formatting = self.workbook.add_format(
            {
                "bold": True,
                "font_color": "white",
                "bg_color": "purple",
        ...
        self.row_formatting = self.workbook.add_format(
            {"text_wrap": True, "font_size": 12}
        )
        self.app = app_name
    ...
    def generate_dictionary(self, root_folder):
        try:
            ...
            logging.info(
                "Definitions Sheet for the Generated Dictionary has been created"
            )
    ...
    def __generate_details_sheet(self):
        ...
        definitition_names = self.ce.map_technical_names_to_definintion_names(
            api_directory, definition_list
        )
        match_list = []
        row_cusur = 1
        for desc_obj in self.parameter_descriptions:
            ...
            for pd in self.parameter_descriptions[desc_obj]:
                ...
                if unique_match not in match_list:
                    ...
                    worksheet.write(
                        row_cusur, 4, pd["description"], self.row_formatting
                    )
                    worksheet.write(
                        row_cusur, 5, "/" + self.app + pd["file"], self.row_formatting
                    )
                    if "endpoint" in pd:
                        worksheet.write(
                            row_cusur, 6, pd["endpoint"], self.row_formatting
                        )
                    items = pd["file"].split("/")
                    if len(items) > 2:
                        contract = items[1]
                        ...
                        worksheet.write(
                            row_cusur, 8, api_endpoints, self.row_formatting
                        )
                    ...
                    if contract in self.classification_report.keys():
                        self.classification_report[contract].append(
                            {"field": desc_obj, "isClassified": isClassified}
                        )
    ...
    def format_api_directory_with_tn(self):
        ...
        for parameter in self.parameter_descriptions:
            x = (
                ((self.parameter_descriptions[parameter][0]["file"]).rsplit("/", 2)[1])
                + ":"
                + parameter
            )
            api_name, tn = x.split(":")
    ...
    def __init_rules_sheet_design(self, worksheet):
        ...
        for i in range(col_sizes["all"]):
            ...
            worksheet.write(
                0, column_cursor, "All{}".format(i + 1), self.title_formatting
            )
            column_cursor += 1
        ...
        for i in range(col_sizes["any"]):
            worksheet.set_column(0, column_cursor, 20)
            worksheet.write(
                0, column_cursor, "Any{}".format(i + 1), self.title_formatting
            )
            column_cursor += 1
        ...
        for i in range(col_sizes["tags"]):
            worksheet.set_column(0, column_cursor, 20)
            worksheet.write(
                0, column_cursor, "Tag{}".format(i + 1), self.title_formatting
            )
            column_cursor += 1
    ...
    def __add_data_validation(self):
        ...
        while row <= self.total_details_rows:
            worksheet.data_validation(
                ...
                {
                    "validate": "list",
                    "source": "='Field Types'!$A2:$A{}".format(
                        self.total_definitions_rows
                    ),
                },
            )
            row += 1
